...
# Run as root: Railway (and most PaaS) attach their persistent volume
# root-owned, so a non-root user cannot create the SQLite file on the mount.
# Hardening for production: chown the volume in an init step and keep USER node.
USER root
EXPOSE 3000
# Railway injects PORT for its router; fall back to 3000 for local/VPS runs.
